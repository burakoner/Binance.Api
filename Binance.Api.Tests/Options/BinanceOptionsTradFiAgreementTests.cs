...
public class BinanceOptionsTradFiAgreementTests
{
    [Fact]
    public async Task SignTradFiOptionsAgreementAsync_UsesCurrentSignedContractAndDeserializesResponse()
    {
        ...
        var result = await client.Options.SignTradFiOptionsAgreementAsync(60_000);

        Assert.True(result.Success);
        Assert.Equal(long.MaxValue, result.Data!.Code);
        Assert.Equal("success", result.Data.Message);
        Assert.Equal(HttpMethod.Post, handler.Method);
        Assert.Equal("/eapi/v1/stock/contract", handler.RequestUri!.AbsolutePath);
        ...
        Assert.Contains(limiter.Requests, item =>
            item.Endpoint == "/eapi/v1/stock/contract" && item.Weight == 50 && item.Signed);
    }

    [Fact]
    public async Task SignTradFiOptionsAgreementAsync_UsesConfiguredReceiveWindow()
    {
        ...
        var result = await client.Options.SignTradFiOptionsAgreementAsync();
    ...
    [Fact]
    public async Task SignTradFiOptionsAgreementAsync_RejectsInvalidReceiveWindowsBeforeTransport()
    {
        var handler = new RecordingHttpMessageHandler("{}");
        using (var client = CreateClient(handler))
        {
            await Assert.ThrowsAsync<ArgumentOutOfRangeException>(() =>
                client.Options.SignTradFiOptionsAgreementAsync(60_001));
        }
        ...
        await Assert.ThrowsAsync<ArgumentOutOfRangeException>(() =>
            configuredClient.Options.SignTradFiOptionsAgreementAsync());
