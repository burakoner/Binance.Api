...
public class BinanceAlgoFuturesQueryTests
{
    [Fact]
    public async Task GetOpenAlgoOrders_UsesCurrentSignedQueryWeightAndResponse()
    {
        ...
        var handler = new RecordingHttpMessageHandler(
            """{"total":2147483648,"orders":[{"algoId":9223372036854775806,"symbol":"ETHUSDT","side":"SELL","positionSide":"SHORT","totalQty":"5.000","executedQty":"0.000","executedAmt":"0.00000000","avgPrice":"0.00","clientAlgoId":"d7096549481642f8a0bb69e9e2e31f2e","bookTime":1649756817004,"endTime":0,"algoStatus":"WORKING","algoType":"VP","urgency":"LOW"}]}""");
        using var client = CreateClient(handler, limiter);

        var result = await client.Algo.Futures.GetOpenAlgoOrdersAsync(60_000);
        ...
        AssertSignedGet(handler, limiter, "/sapi/v1/algo/futures/openOrders");
    ...
    public async Task GetHistoricalAlgoOrders_UsesCurrentFiltersPaginationAndResponse()
    {
        ...
        var result = await client.Algo.Futures.GetHistoricalAlgoOrdersAsync(
            "BTCUSDT",
        ...
        AssertSignedGet(handler, limiter, "/sapi/v1/algo/futures/historicalOrders");
    ...
    public async Task GetAlgoSubOrders_UsesCurrentPaginationAndResponse()
    {
        ...
        var result = await client.Algo.Futures.GetAlgoSubOrdersAsync(
        ...
        AssertSignedGet(handler, limiter, "/sapi/v1/algo/futures/subOrders");
        var query = DecodedQuery(handler);
    ...
    [Fact]
    public async Task FuturesQueries_RejectDocumentedInvalidValues()
    {
        using (var client = CreateClient(new RecordingHttpMessageHandler("{}")))
        {
            await Assert.ThrowsAsync<ArgumentException>(() =>
                client.Algo.Futures.GetAlgoSubOrdersAsync(1, pageSize: 0));
            await Assert.ThrowsAsync<ArgumentException>(() =>
                client.Algo.Futures.GetAlgoSubOrdersAsync(1, pageSize: 101));
            await Assert.ThrowsAsync<ArgumentException>(() =>
                client.Algo.Futures.GetHistoricalAlgoOrdersAsync(symbol: " "));
            await Assert.ThrowsAsync<ArgumentOutOfRangeException>(() =>
                client.Algo.Futures.GetHistoricalAlgoOrdersAsync(side: (BinanceOrderSide)0));
            await Assert.ThrowsAsync<ArgumentException>(() =>
                client.Algo.Futures.GetHistoricalAlgoOrdersAsync(pageSize: 0));
            await Assert.ThrowsAsync<ArgumentException>(() =>
                client.Algo.Futures.GetHistoricalAlgoOrdersAsync(pageSize: 101));
            await Assert.ThrowsAsync<ArgumentOutOfRangeException>(() =>
                client.Algo.Futures.GetOpenAlgoOrdersAsync(60_001));
            await Assert.ThrowsAsync<ArgumentOutOfRangeException>(() =>
                client.Algo.Futures.GetHistoricalAlgoOrdersAsync(receiveWindow: 60_001));
            await Assert.ThrowsAsync<ArgumentOutOfRangeException>(() =>
                client.Algo.Futures.GetAlgoSubOrdersAsync(1, receiveWindow: 60_001));
        }
        ...
        await Assert.ThrowsAsync<ArgumentOutOfRangeException>(() =>
            configuredClient.Algo.Futures.GetOpenAlgoOrdersAsync());
        await Assert.ThrowsAsync<ArgumentOutOfRangeException>(() =>
            configuredClient.Algo.Futures.GetHistoricalAlgoOrdersAsync());
        await Assert.ThrowsAsync<ArgumentOutOfRangeException>(() =>
            configuredClient.Algo.Futures.GetAlgoSubOrdersAsync(1));
    }
