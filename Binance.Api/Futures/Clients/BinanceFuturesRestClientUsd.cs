...
internal partial class BinanceFuturesRestClientUsd(BinanceFuturesRestClient parent) : IBinanceFuturesRestClientUsd
{
    ...
    internal async Task<BinanceTradeRuleResult> CheckTradeRulesAsync(string symbol, decimal? quantity, decimal? quoteQuantity, decimal? price, decimal? stopPrice, BinanceFuturesOrderType type, CancellationToken ct)
    {
        ...
        if (ExchangeInfo == null || LastExchangeInfoUpdate == null || (DateTime.UtcNow - LastExchangeInfoUpdate.Value).TotalMinutes > ApiOptions.UsdtFuturesOptions.TradeRulesUpdateInterval.TotalMinutes)
        ...
        if (symbolData.LotSizeFilter != null || symbolData.MarketLotSizeFilter != null && type == BinanceFuturesOrderType.Market)
        {
            ...
            if (minQty.HasValue && quantity.HasValue)
            {
                ...
                {
                    if (ApiOptions.UsdtFuturesOptions.TradeRulesBehavior == BinanceTradeRulesBehavior.ThrowError)
                    {
