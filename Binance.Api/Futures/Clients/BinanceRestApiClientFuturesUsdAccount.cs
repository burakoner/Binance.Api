...
internal partial class BinanceRestApiClientFuturesUsd
{
    ...
    public Task<RestCallResult<IEnumerable<BinanceSymbolConfiguration>>> GetSymbolConfigurationAsync(string? symbol = null, int? receiveWindow = null, CancellationToken ct = default)
    {
        var parameters = new ParameterCollection();
        parameters.AddOptional("symbol", symbol);
        parameters.AddOptional("recvWindow", _._.ReceiveWindow(receiveWindow));

        return RequestAsync<IEnumerable<BinanceSymbolConfiguration>>(GetUrl(fapi, v1, "symbolConfig"), HttpMethod.Get, ct, true, queryParameters: parameters, requestWeight: 5);
    }
