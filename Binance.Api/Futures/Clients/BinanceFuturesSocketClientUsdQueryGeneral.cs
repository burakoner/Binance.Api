...
internal partial class BinanceFuturesSocketClientUsd
{
    public async Task<CallResult<TimeSpan>> PingAsync(CancellationToken ct = default)
    {
        ...
        var result = await RequestAsync<object>("ws-fapi/v1", $"ping", [], ct: ct).ConfigureAwait(false);
        sw.Stop();
    ...
    {
        var result = await RequestAsync<BinanceServerTime>("ws-fapi/v1", $"time", [], false, ct: ct).ConfigureAwait(false);
