...
/// </summary>
public interface IBinanceRestApiClientFuturesUsdAccount
{
    /// <summary>
    ...
    /// <param name="receiveWindow">The receive window for which this request is active. When the request takes longer than this to complete the server will reject the request</param>
    /// <param name="ct">Cancellation token</param>
    Task<RestCallResult<IEnumerable<BinanceSymbolConfiguration>>> GetSymbolConfigurationAsync(string? symbol = null, int? receiveWindow = null, CancellationToken ct = default);
    ...
    /// <param name="ct">Cancellation token</param>
    /// <returns></returns>
    Task<RestCallResult<IEnumerable<BinanceRateLimit>>> GetOrderRateLimitAsync(int? receiveWindow = null, CancellationToken ct = default);
