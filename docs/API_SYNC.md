...
- The REST route table does not measure WebSocket API completeness. Repository scans expose a separate three-method Spot session-authentication gap (`session.logon`, `session.status`, and `session.logout`), which must be handled before declaring Spot complete.
...
1. Audit and implement the missing Spot WebSocket API session-authentication contract, including signing, key-type behavior, session state, and deterministic tests.
